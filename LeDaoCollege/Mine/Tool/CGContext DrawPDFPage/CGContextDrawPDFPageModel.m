...
- (CGContextDrawPDFPageController *)viewControllerAtIndex:(NSInteger)pageNO {
    // Return the data view controller for the given index.
    if (self.pageSum== 0 || pageNO >= self.pageSum+1){
        return nil;
    }
...
#pragma  mark - 如果要每页的背面显示与正面相同的风格，而不是默认的白，需要设置pageController的doubleSide属性为YES，同时在下面的两个代理方法中设置BackViewController
...
- (NSInteger)pageSum {
    if (_pageSum == 0) {
        _pageSum = CGPDFDocumentGetNumberOfPages(pdfDocument);
    }
    return _pageSum;
}
@end
